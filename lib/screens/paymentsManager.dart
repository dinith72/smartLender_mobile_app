...
//    memList = PaymentManagerDataLoader().getMembers();
    _teamVal = teams[0];
    _date = DateTime.now();
    PaymentManagerDataLoader().getCenterNamesAsync()
...
//    print(center.toString());
    var cenId = _centersMap.keys.firstWhere(
            (k) => _centersMap[k] == center.toString(), orElse: () => null);
    print(cenId);
    PaymentManagerDataLoader().getTeamsAsync(cenId)
    .then((Map<String,String> results){
      setState(() {
//        teams.clear();
        _teamsMap = results;
...
//    selMem.add(tmemId);
  ...
  teamOnChanged(String team){
    ...
    setState(() {
      _teamVal = team;
      PaymentManagerDataLoader().getMembersAsync(teamId)
