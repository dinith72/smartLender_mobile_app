...
//  String _dateStr = 'Today';
  ...
  Map<int , CenterInfo> centerMap = new Map();
  List<CenterInfo> centerList = new List();
...
//    _payAmnt = HomeController().getTotalPayments(_date);
     HomeController().getCenterInfo(_date, _nic)
      .then((Map<int,CenterInfo> results){
       print('center info res');
       print(results);
       if(results != null){
         this.centerMap = results;
         setState(() {
           for(var id in results.keys){
             centerList.add(results[id]);
           }
           centerList.forEach((cen){
             if(cen.collections != null){
               _colAmnt = _colAmnt + cen.collections;
             }
             if(cen.payments != null){
               _payAmnt = _payAmnt + cen.payments;
             }
             print(cen.name);
             print(cen.collections.toString());
             print(cen.payments.toString());
           });
           print(_colAmnt);
           print(_payAmnt);
         });
                      ...
                      delegate: new SliverChildListDelegate(
                          <Widget>[
                            new SwitchListTile(
                                ...
                                onChanged:
                                    (bool value){print(_sendLoacation);}
                            ),
                            new Row(
