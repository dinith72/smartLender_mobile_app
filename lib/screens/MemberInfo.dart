...
//    MemberInfoController().getMemberDetails().
...
//    _members = MemberInfoController().getMembers();
    MemberInfoController().getMembersAsync()
    .then((Map<String,String> results ){
      memberMap = results;
      for(var id in results.keys){
        _members.add(results[id]);
      }
    });
...
//  _members.clear();
    ...
    _repayement = '';
    _attendence = '';
    var nic = memberMap.keys.firstWhere(
            (k) => memberMap[k] == memName.toString(), orElse: () => null);
    print(nic);

     setState(() {
//       _memberval = memName.toString();
       print(memName.toString());
       MemberInfoController().getMemberDetails(nic)
       .then((List<Entry> results){
          this.memberDetials = results;
          if(results != null){
            MemberInfoController().getRepayment()
                .then((double pmnt) {
              setState(() {
                _repayement = pmnt.toString().substring(0,4) + ' %';
              });
       ...
       MemberInfoController().getAtendence(nic)
           .then((double att) {
         if(att != null){
           setState(() {
             _attendence = att.toString()  + ' %';
           });
